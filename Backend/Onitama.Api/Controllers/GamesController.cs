...
namespace Onitama.Api.Controllers
{
    ...
    public class GamesController : ApiControllerBase
    {
        ...
        [HttpGet("{id}/possible-moves-for-player/{playerId}")]
        [ProducesResponseType(typeof(IReadOnlyList<IMove>), StatusCodes.Status200OK)]
        [ProducesResponseType(typeof(ErrorModel), StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public IActionResult GetAllPossibleMovesFor(Guid id, Guid playerId)
        {
            IReadOnlyList<IMove> moves = _gameService.GetAllPossibleMovesFor(id, playerId);
            List<MoveModel> models = moves.Select(move => _mapper.Map<MoveModel>(move)).ToList();
            return Ok(models);
        }
